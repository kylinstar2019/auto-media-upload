...
def custom_static(filename):
    if os.path.exists(os.path.join(frontend_dist, "assets", filename)):
...
def favicon():
    if os.path.exists(os.path.join(frontend_dist, "favicon.ico")):
...
@app.route("/vite.svg")
def vite_svg():
    if os.path.exists(os.path.join(frontend_dist, "vite.svg")):
...
def upload_file():
    if "file" not in request.files:
        return jsonify(
            {"code": 200, "data": None, "msg": "No file part in the request"}
        ), 400
        ...
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            for row in rows:
                ...
                if row_dict.get("file_path"):
                    file_path_parts = row_dict["file_path"].split(
                        "_", 1
                    )  # 只分割第一个下划线
...
def getAccounts():
    ...
    try:
        ...
        return jsonify(
            {"code": 500, "msg": f"获取账号列表失败: {str(e)}", "data": None}
        ), 500
...
def delete_file():
    ...
    if not file_id or not file_id.isdigit():
        return jsonify(
            {"code": 400, "msg": "Invalid or missing file ID", "data": None}
        ), 400
    ...
    try:
        ...
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            if not record:
                return jsonify(
                    {"code": 404, "msg": "File not found", "data": None}
                ), 404
...
def delete_account():
    ...
    try:
        ...
        with sqlite3.connect(Path(BASE_DIR / "db" / "database.db")) as conn:
            ...
            if not record:
                return jsonify(
                    {"code": 404, "msg": "account not found", "data": None}
                ), 404
        ...
        return jsonify(
            {"code": 200, "msg": "account deleted successfully", "data": None}
        ), 200
...
def login():
    ...
    thread = threading.Thread(
        target=run_async_function, args=(type, id, status_queue), daemon=True
    )
    thread.start()
    response = Response(
        sse_stream(
...
def updateUserinfo():
    ...
    try:
        ...
        return jsonify(
            {"code": 200, "msg": "account update successfully", "data": None}
        ), 200
...
def upload_cookie():
    try:
        if "file" not in request.files:
            return jsonify(
                {"code": 500, "msg": "没有找到Cookie文件", "data": None}
            ), 400

        file = request.files["file"]
        if file.filename == "":
            return jsonify(
                {"code": 500, "msg": "Cookie文件名不能为空", "data": None}
            ), 400

        if not file.filename.endswith(".json"):
            return jsonify(
                {"code": 500, "msg": "Cookie文件必须是JSON格式", "data": None}
            ), 400
        ...
        if not account_id or not platform:
            return jsonify(
                {"code": 500, "msg": "缺少账号ID或平台信息", "data": None}
            ), 400
        ...
        return jsonify(
            {"code": 500, "msg": f"上传Cookie文件失败: {str(e)}", "data": None}
        ), 500
...
def download_cookie():
    try:
        ...
        return jsonify(
            {"code": 500, "msg": f"下载Cookie文件失败: {str(e)}", "data": None}
        ), 500
